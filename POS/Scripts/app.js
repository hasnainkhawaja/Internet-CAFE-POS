...
function calculateItemRateTax() {
    $itemRate = $("#itemRate");
    $taxId = $("#taxId");
    $itemratetax = $("#itemratetax");
    var items = $taxId.val().split('-');
    var tax = items[1];
    if ($itemRate.val().length > 0) {
        if (tax != 0) {
            $itemratetax.val(parseFloat($itemRate.val()) + parseFloat($itemRate.val()) / parseFloat(tax));
        }
        else {
            $itemratetax.val(parseFloat($itemRate.val()))
        }
    }
    else {
        $itemratetax.val(0);
...
function calculateRate()
{
    ...
    var items = $taxId.val().split('-');
    var tax = items[1];
    if ($itemratetax.val().length > 0) {
        if (tax != 0) {
            $itemRate.val(parseFloat($itemratetax.val()) - (parseFloat($itemratetax.val()) / parseFloat(tax)) + 1)
        }
        else {
            $itemRate.val(parseFloat($itemratetax.val()))
        }
    }
    else {
        $itemRate.val(0);
